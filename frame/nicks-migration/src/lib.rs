...
// re-export the genesis config of the deprecated storage in order to make it usable from outside
#[cfg(feature = "std")]
pub use deprecated::GenesisConfig;

pub mod deprecated {
	use crate::Trait;
	use frame_support::{decl_module, decl_storage, traits::{Currency, ReservableCurrency, Get}};
	use sp_std::prelude::*;
	...
	decl_storage! {
		...
		add_extra_genesis {
			config(nicks): Vec<(T::AccountId, Vec<u8>)>;
			build(|config| {
				let deposit = T::ReservationFee::get();

				for (acc, nick) in &config.nicks {
					T::Currency::reserve(acc, deposit.clone())
						.expect("reservation for nick should not fail in genesis");
					<NameOf<T>>::insert(acc, (nick.clone(), deposit));
				}
			})
		}
	}
...
pub mod migration {
	...
	pub fn migrate_to_v2<T: Trait>() -> frame_support::weights::Weight {
		...
		if PalletVersion::get() == StorageVersion::V1Bytes {
			...
			frame_support::debug::info!(" >>> Updating NicksMigration storage. Migrating {} nicknames...", count);
			...
			frame_support::debug::info!(" <<< NicksMigration storage updated! Migrated {} nicknames ✅", count);
